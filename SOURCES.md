...
- A manifest row may carry a note after its closing pipe (`| ok (1p) |  ← typed core read from here`). `tools/sync.py` reads the cells and ignores the note — `MANIFEST_ROW` was fixed on 2026-09-03 to tolerate it, after seven annotated rows had silently dropped out of `parse_manifest()`. Keep notes after the pipe, never as an extra cell.
