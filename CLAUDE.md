...
- **The v1 summary strings are still the ones on screen.** `maxLTV`, `maxMileage`, `gapMax` and the rest still render the cards, and they still show a bank's *best* cell — truist's 155%, dfc's 135% — while the tools now read the typed floor and its conditions. So the card and the calculator can disagree about the same lender, and the calculator is the one to trust. Reconciling them is a UI change nobody has asked for yet.
...
- **Also waiting on a decision:** whether the v1 summary strings on the cards should be rendered from the typed core, so the card and the calculator stop disagreeing about the same lender; whether a rate floor becomes a top-level field; whether provenance moves into `lenders.json` per field
