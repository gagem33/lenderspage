...
**Known issues**
- **56 proposed changes are sitting undecided in `sync/proposals/`** from the 2026-08-26 corpus sweep. Nothing is applied. `python3 tools/sync.py diff sync/proposals/<file>` to review, `approve` then `apply` to write.
...
- **Rate sheets are still unmodelled and it has now cost data on at least six lenders.** `regional` publishes a 112-cell consumer rate grid, `fifththird` a full buy-rate table, and `gls`, `exeter`, `westlake` and `santander` all publish rate floors or bands. EXTRACTION_GUIDE §8 says flag rather than bolt on — this is spec item #3 and it is the largest single thing the app does not capture.
...
- **`td` and `dfc` filenames are wrong, not their data.** Both documents state their own dates internally (TD footer 06/30/2026; DFC header "Revision Date: 8/13/2025"). Renaming the two Drive files and updating their SOURCES.md rows would let both acknowledgements be deleted rather than carried forever.
