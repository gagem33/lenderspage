...
- **Nothing honours `prefers-reduced-motion` any more.** The page's one such block guarded `.detail-section-wrap`, which the rewrite deleted, so the guard was removed with it. Eleven `transition:` rules and the `modalIn` keyframe are still unguarded — as they were before, since that block never covered them. Restoring it properly is a small change, not a cleanup.
...
- **Cheap and unblocked:** decide whether the two unsourced `chase` values stay or go; decide whether `prefers-reduced-motion` should be honoured for real
