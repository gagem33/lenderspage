...
- **Next — the v2 core.** The conversation happened 2026-08-28 and produced three decisions (typed core beside the detail; conditions the tools can evaluate; eligibility-first ranking) — all written into DATA.md §2. What is left is building it: parse what parses, leave the rest `null`, then fill the gaps against the PDFs through the sync's gate. `truist` and `cps` first, per DATA.md §4.2
