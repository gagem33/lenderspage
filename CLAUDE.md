...
- **Navigation speed (spec #10), 2026-08-26.** `/` or `⌘K`/`Ctrl+K` opens a jump-to box that reaches any lender, tool or view — ranked, arrow keys, Enter. Single keys open the six tools (`i b l d s y`), `c` is the compare table, `[` and `]` step through lenders, `?` lists the lot. Sidebar search now ranks name matches first and says *where* a hit landed (`in Vehicle Eligibility`), which matters because it searches every section's text. Shortcuts never fire while a field has focus; `⌘K` is the one exception, so it works from inside the search box
...
- ~~UI restyle: Gage likes it but thinks it can be better.~~ **Done 2026-08-27** — Desk identity plus the card layout. Still open: roughly 39 CSS rules for the removed sidebar and detail page are now dead and were left in place rather than deleted mid-rewrite. Inert, but worth a cleanup pass.
...
- **Next:** nothing outside the v2 schema. #3 (rates), #4 (freshness) and #10 (navigation) all shipped 2026-08-26
