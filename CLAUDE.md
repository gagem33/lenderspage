...
- **Next — the remaining 18 typed cores.** `truist` and `cps` were built 2026-08-28 off their source PDFs and both tools read them; `tools/core.py` validates and self-tests. Each remaining lender is a PDF read: the numbers mostly parse, but front-vs-total LTV and the conditions attached to limits do not. 8 lenders cannot say whether their LTV is front or total (DATA.md §3.2) and stay `null` until someone looks
