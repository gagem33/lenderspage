...
- ~~LTV calc and deal structurer parse the `maxLTV` string directly.~~ **Fixed 2026-08-28** — both go through `lenderLimit()`, which returns the typed value where a lender has a core and falls back to the string parse where it does not. See ARCHITECTURE.md §3.4. The fallback is what keeps the other 18 working, and it goes away as they are typed.
...
- **Next — nothing in the schema.** All 20 lenders carry a typed core as of 2026-08-28, read from their own PDFs; 78 resolution cases pass in both `core.py` and the browser. What is left is Gage's: the rep conversations below, now with three more (`westlake`'s Prime-vs-Independent sheet decides whether its LTV can be typed at all; `regional` publishes no total-LTV cap; `td`, `ally` and `kia` keep their LTV in a different file from their effective date)
